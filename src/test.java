...
public class test {
	...
	//	publishedUIDs.add(123); // 0
		...
		//Execute Query 
		String Query = "PREFIX ssn: <http://www.w3.org/2005/Incubator/ssn/ssnx/ssn/> "+
						"PREFIX DUL: <http://www.ontologydesignpatterns.org/ont/dul/DUL.owl/> " +
						"PREFIX ex: <http://www.sensordescription.de/inrdf/> " +
						"SELECT ?v1 ?v2 " +
						"WHERE {?s1 a ssn:Sensor . " +
						"?s2 a ssn:Sensor . " +		
						"?s1 ex:type ex:TemperatureSensor . " +
	...
	private static Model generateRDFModel(Model newModel, ArrayList<Integer> publishedUIDs, byte[] b) {
		...
		while(it.hasNext()){
			...
			Selector selector2 = new SimpleSelector(Subject, null, (RDFNode) null);  		
			...
			while(it2.hasNext()){
				...
				linker.put(Subject, fastRDFNode);				
			}			
			it2.close();		
		}
		it.close();
		...
		ArrayList<Resource> updateNode = new ArrayList<>();
		...
		//remove where object is blank node
		newModel.remove(toBeDeleted);
		
		//remove updated blank nodes
		for(Resource res: updateNode){
			newModel.removeAll(res, null, (RDFNode) null);
		}			
